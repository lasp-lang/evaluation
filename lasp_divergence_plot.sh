...
main(_) ->
    ValidDirectories = ordsets:from_list(["ad_counter_divergence"]),
...
average(Simulation, EvalIds) ->
    lists:foldl(
        fun(EvalId, Map) ->
            ClientNumber = get_client_number(EvalId),
            ...
            orddict:store(ClientNumber, Avg, Map)
        end,
        orddict:new(),
        EvalIds
    ).
...
%% @private
get_client_number(EvalId) ->
    Tokens = string:tokens(EvalId, "_"),
    ClientNumber = lists:last(Tokens),
    list_to_integer(ClientNumber).
...
write_to_file(Simulation, Name, Map) ->
    ...
    lists:foreach(
        fun({Clients, Divergence}) ->
            append_to_file(InputFile, Clients, Divergence)
        end,
        Map
    ),
